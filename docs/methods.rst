...
message -str – This is the message that will be sent to the user
...
This is a method used to push notifications to a user.  NOTE: This can only be used during cron runs.  It is mainly intended to inform users that their cron job is not running correctly
...
.. code-block:: python
    ...
    elm = ELMSDK(instance_key)
    elm.push_notification('Your long running task was unable to complete.  Please return to elmware and change the configuration.')
...
url – This is a externally facing URL that is able to request incoming web traffic.  It can be used to process callbacks. In contrast to the find_callback_url function, though, the url generated by this function will always be the same (across users and across tools).  
...
This method is used to find a callback url that can be used for webhooks from external resources.  For example, the oauth process requires an incoming request from an external source. Using this method, a container is able to open up an external url to incoming traffic, and then later access details of the latest request that was made to that url.  For this method the callback MUST then be made by the authenticated user that was using the tool.  This is used in cases where a service requires that callback urls are whitelisted, and so it becomes impractical to generate a new callback url each time.
...
.. code-block:: python
    ...
    elm = ELMSDK(instance_key)
    url_info = elm.find_callback_redirect_url()
    some_function_to_send_to_external_resource(url_info['url'])
    some_function_to_store_the_retreival_key(url_info['key'])
